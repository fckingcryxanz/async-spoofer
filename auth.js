// Вставь сюда ссылку, полученную в Шаге 2 на Discord Developer Portal:
...
export function checkAuthAndRoute() {
    ...
    // 1. Если Дискорд вернул код авторизации (?code=...)
    if (code) {
        // Убираем код из строки URL для красоты
        window.history.replaceState({}, document.title, window.location.pathname);
        
        // Сохраняем сессию (в будущем тут будет запрос к твоему серверу)
    ...
    // 3. Обрабатываем текущий URL (Clean Routing)
    const currentPath = window.location.pathname;

    if (!isAuthenticated) {
        // Если не залогинен — принудительно рисуем окно входа на любой странице
        renderLoginScreen();
    } else {
        // Если залогинен — убираем окно входа (если оно было) и пускаем к панелям
        const loginWrapper = document.getElementById('login-screen-element');
        if (loginWrapper) loginWrapper.remove();

        // Твоя логика переключения вкладок в зависимости от URL:
        if (currentPath === '/settings') {
            document.getElementById('tab-settings')?.click(); // Клик по вкладке настроек
        } else if (currentPath === '/studio') {
            document.getElementById('tab-studio')?.click(); // Клик по вкладке студии
        }
    }
}

// Функция отрисовки меню входа (С измененным расположением блоков)
function renderLoginScreen() {
    // Проверяем, вдруг оно уже отрисовано
    ...
    loginBlock.innerHTML = `
        <div class="login-container-split">
            ...
            <!-- ЛЕВАЯ СТОРОНА: ВХОД (Изменили позицию) -->
            <div class="auth-main-card">
                ...
                <!-- Ссылка на авторизацию приложения бота -->
                ...
                <!-- Твоя ссылка на ДС Сервер -->
                <a href="https://discord.gg/GjnubgmcYx" target="_blank" class="discord-join-btn">
                    Join our Discord Server
                </a>
            </div>

            <!-- ПРАВАЯ СТОРОНА: СВОЙСТВА И ОПИСАНИЕ (Изменили позицию) -->
            <div class="info-side-card">
                <div>
                    <!-- Используем твою ссылку на новый логотип -->
                ...
                <!-- Статистика вынесена в нижнюю строчку -->
